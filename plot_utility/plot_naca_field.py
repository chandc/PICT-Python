...
f = sys.argv[1]; alpha = float(sys.argv[2]) if len(sys.argv) > 2 else 20.0; d = np.load(f); H = d["hist"]; t_end = H[-1, 0]
...
panel(ax[0, 0], Wv, np.linspace(-8, 8, 33), "RdBu_r", f"vorticity, near field, t = {t_end:.0f}", (-0.5, 3.0), (-2.0, 1.0))
...
n2 = int(0.6 * len(H)); plt.suptitle(f"NACA0012, alpha = {alpha:.0f} deg, Re = 100, t = {t_end:.0f}: C_D {H[-1,1]:.3f}, C_L {H[-1,2]:.3f} (instantaneous)", fontsize=12)
out = f"figures/naca0012_a{int(alpha)}_field_t{int(round(t_end))}.png"; plt.tight_layout(); plt.savefig(out, dpi=110); print("wrote", out)
