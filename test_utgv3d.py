...
def build(n, nz, nu, dt, sgs="none"):
    ...
    s = PISO25(m, nz, L, nu, dt, BC(m), BC(m), BC(m), BC(m), n_nonorth=1); s.sgs_model = sgs
    x, y = m.centroid.T; z = s.z
    s.u[:] = np.sin(x)[:, None] * np.cos(y)[:, None] * np.cos(z)[None, :]
    s.v[:] = -np.cos(x)[:, None] * np.sin(y)[:, None] * np.cos(z)[None, :]
    s.p[:] = (np.cos(2 * x) + np.cos(2 * y))[:, None] * (np.cos(2 * z) + 2)[None, :] / 16
    return m, s
...
if __name__ == "__main__":
    ...
    if "A" in parts:
        import os; Re = float(os.environ.get("TGV_RE", 100)); T = float(os.environ.get("TGV_T", 10)); n = int(os.environ.get("TGV_N", 32)); nz = int(os.environ.get("TGV_NZ", 32)); dt = float(os.environ.get("TGV_DT", 0.02))
        ...
        np.savez(f"results/tgv3d_re{Re:.0f}_n{n}_nz{nz}_dt{dt}_{sgs}.npz", t=t, E=E, Z=Z, eps=eps, nu=nu, nut_mean=ntm, nut_max=ntx)
    ...
    if "B" in parts:
        ...
        for nz in (4, 8, 16):
            ...
            e = np.sqrt(float((m.vol[:, None] * ((s.u - r.u[:, ::step]) ** 2 + (s.v - r.v[:, ::step]) ** 2 + (s.w - r.w[:, ::step]) ** 2)).sum()) * L / nz)
    ...
    if "C" in parts:
        ...
        for n in (16, 32, 64):
            ...
            np.add.at(ub, key_f, r.u); np.add.at(vb, key_f, r.v); np.add.at(wb, key_f, r.w); np.add.at(cnt, key_f, 1)
            ub /= cnt[:, None]; vb /= cnt[:, None]; wb /= cnt[:, None]
            e = np.sqrt(float((m.vol[:, None] * ((s.u - ub[key_c]) ** 2 + (s.v - vb[key_c]) ** 2 + (s.w - wb[key_c]) ** 2)).sum()) * L / nz)
