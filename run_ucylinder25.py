...
nu = 1.0 / a.Re
s = PISO25(m, a.nz, a.Lz, nu, a.dt, BC(m, ku, vu), BC(m, kv, vv), BC(m, kw, vw), BC(m, kp, vp), n_nonorth=3)
C = m.centroid; blob = np.exp(-((C[:, 0] - 1.0) ** 2 + C[:, 1] ** 2))
s.u[:] = 1.0; s.v[:] = 0.05 * blob[:, None]
s.w[:] = a.eps3d * blob[:, None] * np.sin(2 * np.pi * s.z / a.Lz)[None, :]
...
def forces():
    gus = s.u[wo] / dn[:, None]; gvs = s.v[wo] / dn[:, None]
    gux, guy = gus * e_in[:, 0, None], gus * e_in[:, 1, None]; gvx, gvy = gvs * e_in[:, 0, None], gvs * e_in[:, 1, None]
    pf = s.p[wo]; txx = 2 * nu * gux; tyy = 2 * nu * gvy; txy = nu * (guy + gvx)
...
def e3d():
    ...
    for f in (s.u, s.v, s.w):
        fh = np.fft.rfft(f, axis=1) / s.nz; tot += float((m.vol[:, None] * 2 * np.abs(fh[:, 1:]) ** 2).sum())
    return 0.5 * tot
...
for k in range(nsteps):
    ...
    if (k + 1) % a.report == 0:
        ...
        print(f"  t={s.time:7.2f}  Cd {w[:,1].mean():.4f}  Cl {w[:,2].mean():+.4f} (amp {0.5*(w[:,2].max()-w[:,2].min()):.4f})  Cd_p {w[:,3].mean():.4f}  E3d {hist[k,4]:.3e}  |w|max {np.abs(s.w).max():.2e}  ({(time.time()-t0)/(k+1)*1e3:.1f} ms/step)", flush=True)
...
if a.out: np.savez(a.out, hist=hist, nodes=m.nodes, cells=m.cells, nvert=m.nvert, centroid=m.centroid, vol=m.vol, u=s.u, v=s.v, w=s.w, p=s.p, btag=m.btag, z=s.z)
