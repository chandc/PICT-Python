"""NACA0012 airfoil at angle of attack on the structured C-grid (meshes/gen_naca_cgrid.py). Derived from
run_ucylinder.py; differences: tags Inlet (far field, u = 1, v = 0 Dirichlet), Outlet (p = 0), Airfoil (no-slip);
no symmetry planes; coefficients per chord (= 1): C_D = 2 F_x, C_L = 2 F_y; St from C_L zero-crossings with
the chord as length. Reference: HydroGym NACA0012_2D_Re100_AOA20 (m-AIA LBM) unperturbed means C_D 0.577, C_L 0.783.

ORIGINAL HEADER (cylinder):
...
T_IN, T_OUT, T_CYL = inv["Inlet"], inv["Outlet"], inv["Airfoil"]; T_FS = -1   # no symmetry planes on the C-grid
bt = m.btag[m.bfaces]
nb = m.nbface
...
if a.steady:
    sys.exit("--steady (mirror symmetrisation) does not apply to a lifting body")
...
print(f"NACA0012 {a.mesh}: {m.ncell} cells ({int((m.nvert==4).sum())} quads), {len(wall)} wall faces, Re={a.Re}, dt={a.dt}, {nsteps} steps to T={nsteps*a.dt:.1f}", flush=True)
