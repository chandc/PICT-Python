...
class MultiBlockPISO:
    def __init__(self, domain, nu, dt, corrector_steps=2, tol=1e-4, time_scheme='bdf2',
        ...
        self.picard_residual = np.inf   # last measured Picard change
        ...
        # default to. Measured on the five-domain BFS at Re=300, restarting each case from the
        # same converged field so only the tolerance varied:
        #
        #   rtol    it/step   s/step   divF(int)   x_r/S    max|u-u_ref|
        #   1e-12      1757    0.505     6.1e-15   6.714             --
        #   1e-08      1568    0.457     1.8e-12   6.714        2.5e-06
        #   1e-06       861    0.269     2.0e-10   6.714        1.0e-04
        #   1e-04        48    0.059     5.0e-08   6.720        9.0e-03
        #
        # Reattachment is unchanged to four figures down to 1e-6 and moves 0.09% at 1e-4, for
        # 8.6x less work. The iteration collapse (1757 -> 48) shows how much of the old cost
        # bought digits below the O(h^2) ~ 1e-3 discretisation error.
        #
        # NOTE for the verification suites: tests that compare fields at MACHINE precision
        # must pass a tight tol explicitly. At 1e-4 two runs agree to ~1e-3, not ~1e-15, and
        # a machine-precision assertion would be checking the solver's noise floor rather
        # than the property it is meant to gate.
        self.tol = tol
        self.Js, self.ms = [], []
    ...
    def _step_impl(self):
        ...
        for _k in range(self.picard_iters):
            ...
            # can resolve. Iterating past `tol` refines a linearisation to a precision the
            # solve underneath it does not have. Measured on the steady five-domain BFS at
            # Re=300: sweep 1 -> 2 changes the answer by 1.2e-08 relative and 2 -> 3 -> 4 by
            # ~5e-10 more, each costing ~70% more time, against a 1e-4 solver tolerance.
            # NOTE this is a STEADY-flow measurement. The Picard sweep exists to remove the
            # O(dt) lag in the convecting velocity, which matters for TIME-ACCURATE unsteady
            # runs; there the count may genuinely be needed and this test will simply not
            # trigger, because the residual stays above tol.
            if convect is not None:
                scale = max(max(np.abs(np.concatenate([a.ravel() for a in c.values()])).max()
                                for c in new_convect), 1e-30)
                rel = max(np.abs(np.concatenate([new_convect[i][b].ravel() for b in
                                                 range(len(self.d.blocks))])
                                 - np.concatenate([convect[i][b].ravel() for b in
                                                   range(len(self.d.blocks))])).max()
                          for i in range(3)) / scale
                self.picard_residual = rel
                if rel < self.tol:
                    convect = new_convect
                    break
            convect = new_convect
