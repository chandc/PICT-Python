...
class Block:
    ...
    def __init__(self, shape, x, y, z, h, faces=None, period=(1.0, 1.0, 1.0),
        ...
        # OPTIONAL: (gx, gy, gz, offset) -- the SEPARABLE background node distribution this
        # block was cut from, and where it starts in it. Supplying it is what makes a grid with
        # an obstacle satisfy the geometric conservation law; see `Domain.pad_coords`.
...
class Domain:
    ...
    def validate(self):
        ...
        problems += self.check_spacing()
        return problems

    # Adjacent cells may differ in size by at most this factor before the discretisation stops
    # being smooth enough for the formal order of accuracy to mean anything. 1.2 is the usual
    # engineering limit; 2.0 is where it becomes a defect rather than a compromise.
    SPACING_WARN = 1.2
    SPACING_FAIL = 2.0

    def check_spacing(self, warn=None, fail=None):
        """Cell-size ratios INSIDE each block and ACROSS every seam, measured as DISTANCE.

        WHY THIS EXISTS. The seam check above states its invariant on node COINCIDENCE, and
        deliberately so -- a connected axis's spacing comes from the global cell count, not one
        block's. That left the PHYSICAL spacing unchecked, and a grid can satisfy every other
        invariant while jumping cell size by a factor of five at a seam. Not hypothetical: the
        square-cylinder grid ran with dx going 0.0323 -> 0.1500, a 4.65x jump at the body's
        trailing edge, exactly where the shear layers separate. `validate()` returned 0 problems
        and the pressure rang at the Nyquist wavelength immediately downstream.

        SPACING IS EUCLIDEAN DISTANCE BETWEEN ADJACENT NODES, not the difference of one Cartesian
        component along an index line. Those are the same thing only on an axis-aligned grid. The
        first version used the component difference and reported 3.00x FAILs on the cylinder
        O-grid wherever an azimuthal line runs nearly tangent to the x axis and the x-difference
        collapses toward zero -- pure artefact, on a grid whose real ratios are all ~1.07.

        Measured over the whole block, not one edge line: a stretched curvilinear grid's worst
        ratio need not sit on the boundary.
        """
        warn = self.SPACING_WARN if warn is None else warn
        fail = self.SPACING_FAIL if fail is None else fail
        out = []

        def spacing(blk, axis):
            d = None
            for c in (blk.x, blk.y, blk.z):
                dc = np.diff(np.asarray(c, dtype=float), axis=axis)
                d = dc * dc if d is None else d + dc * dc
            return np.sqrt(d)

        def worst_ratio(sp, axis):
            if sp.shape[axis] < 2:
                return 1.0, None
            a = np.take(sp, np.arange(sp.shape[axis] - 1), axis=axis)
            b = np.take(sp, np.arange(1, sp.shape[axis]), axis=axis)
            eps = 1e-30
            r = np.maximum(b / np.maximum(a, eps), a / np.maximum(b, eps))
            k = np.unravel_index(np.nanargmax(r), r.shape)
            return float(r[k]), k
        ...
        for b, blk in enumerate(self.blocks):
            for axis in range(3):
                if blk.shape[axis] < 3:
                    continue
                r, k = worst_ratio(spacing(blk, axis), axis)
                if r > warn:
                    lvl = "FAIL" if r > fail else "warn"
                    out.append(f"[{lvl}] block {b} axis {axis}: cell size jumps {r:.2f}x at "
                               f"index {k}; adjacent cells should stay within {warn:.2f}x")

        for c in self.connections:
            A, B = self.blocks[c.ba], self.blocks[c.bb]
            aa, sa = face_axis_side(c.fa)
            ab, sb = face_axis_side(c.fb)
            if A.shape[aa] < 2 or B.shape[ab] < 2:
                continue
            ca = np.take(spacing(A, aa), -1 if sa == 1 else 0, axis=aa)
            cb = np.take(spacing(B, ab), 0 if sb == 0 else -1, axis=ab)
            try:
                cb = c.align(cb)
            except Exception:
                continue
            if ca.shape != cb.shape:
                continue
            eps = 1e-30
            rm = float(np.nanmax(np.maximum(cb / np.maximum(ca, eps),
                                            ca / np.maximum(cb, eps))))
            if rm > warn:
                lvl = "FAIL" if rm > fail else "warn"
                out.append(f"[{lvl}] {c}: cell size jumps {rm:.2f}x ACROSS the seam; "
                           f"adjacent cells should stay within {warn:.2f}x")
        return out
    def __repr__(self):
    ...
    def _pad_coords_background(self, b, width, bg):
        """Ghost coordinates cut straight out of the SEPARABLE background distribution.

        WHY THIS EXISTS. Every block derives its metrics from its own padded coordinates. A
        block abutting an obstacle has a WALL there, so it was not padded and fell back to
        ONE-SIDED differences along that axis, while the block across the seam used CENTRAL
        differences with real neighbour data. The GCL sum at a reentrant corner mixes metric
        terms from both and stops telescoping, so a uniform flow acquires nonzero divergence:
        measured 1.81e+00 on the square cylinder and 1.93e+00 on the five-domain BFS, and about
        0.5% spurious velocity manufactured out of an undisturbed freestream.

        Neither ghost FILL fixes that. The ghost inside the solid has to be the continuation of
        the GLOBAL grid, which no block knows locally -- for an H-grid it is the obstacle's own
        interior tensor grid, geometrically well defined and stored by nobody. So the builder
        hands it over instead: `background = (gx, gy, gz, offset)` are the node distributions of
        the WHOLE background rectangle, solid region included, and where this block starts in
        them. Every block then slices the SAME arrays, so ghost coordinates are single-valued
        everywhere -- including inside the body -- and every block uses the identical central
        stencil. That is exactly what the metric identity needs.

        A face with no data beyond it in the background is a TRUE domain boundary; it stays
        unpadded and one-sided, as the single-block code does.

        Only valid for a separable (tensor-product) background, which is what an H-grid around a
        box is. Blocks without `background` keep the neighbour-walking path above.
        """
        blk = self.blocks[b]
        g, off = list(bg[:3]), tuple(bg[3])
        lo, hi, idx = [0, 0, 0], [0, 0, 0], []
        vals = []
        for a in range(3):
            n, o, ga = blk.shape[a], off[a], np.asarray(g[a], dtype=float)
            periodic = blk.faces[face_id(a, 0)] == "periodic"
            if periodic:
                lo[a] = hi[a] = width
                k = np.arange(o - width, o + n + width)
                m = len(ga)
                # Wrap, and shift by whole periods -- coordinates ramp and then jump back, so a
                # wrapped ghost must carry the period or it collapses the Jacobian at the seam.
                vals.append(ga[k % m] + (k // m) * blk.period[a])
            else:
                lo[a] = min(width, o)
                hi[a] = min(width, len(ga) - (o + n))
                vals.append(ga[o - lo[a]: o + n + hi[a]])
        X, Y, Z = np.meshgrid(vals[0], vals[1], vals[2], indexing="ij")
        return X, Y, Z, lo, hi
    ...
    def pad_coords(self, b, width=2):
        ...
        """
        blk0 = self.blocks[b]
        bg = getattr(blk0, "background", None)
        if bg is not None:
    ...
    def padded_geometry(self, b, width=1):
        ...
        # and the result must line up with `pad_field`, which pads ONLY periodic and connected
        # faces. A block carrying a `background` also gets ghosts across obstacle WALLS, which
        # no field can supply, so trim back to the extent the fields actually have or the two
        # arrays no longer broadcast.
        seam = ("periodic", "connected")
        tgt_lo = [min(lo[a], width if blk.faces[face_id(a, 0)] in seam else 0) for a in range(3)]
        tgt_hi = [min(hi[a], width if blk.faces[face_id(a, 1)] in seam else 0) for a in range(3)]
        trim = [lo[a] - tgt_lo[a] for a in range(3)]
        trimh = [hi[a] - tgt_hi[a] for a in range(3)]
