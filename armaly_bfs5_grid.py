...
def bfs5_domain(nx_in=28, nx_re=56, nx_rc=56, ny_lo=24, ny_up=26, nz=16):
    ...
    # The BACKGROUND distribution: both axes as ONE array each, spanning the whole rectangle
    # INCLUDING the solid step (x_in x y_lo). Handing it to every block lets the ones abutting
    # the step use a central metric stencil, which the geometric conservation law needs at the
    # step's reentrant corner -- without it a uniform flow picked up divergence 1.93e+00 here.
    # See reference/reentrant_corner_gcl.md.
    GX = np.concatenate([x_in, x_re, x_rc])
    GY = np.concatenate([y_lo, y_up])

    def mk(xs, ys, xoff, yoff):
        ...
        b = Block((len(xs), len(ys), nz), X, Y, Z, (hx, hy, hz),
                  ...
                  background=(GX, GY, z, (xoff, yoff, 0)))
        b.faces[face_id(2, 0)] = b.faces[face_id(2, 1)] = "periodic"
        return b

    xo_re, xo_rc = nx_in, nx_in + nx_re
    blocks = [mk(x_in, y_up, 0, ny_lo), mk(x_re, y_up, xo_re, ny_lo), mk(x_re, y_lo, xo_re, 0),
              mk(x_rc, y_up, xo_rc, ny_lo), mk(x_rc, y_lo, xo_rc, 0)]
