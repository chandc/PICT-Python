...
def classify(d, nblk=None):
    """(block, face) -> 'cylinder' | 'inflow' | 'outflow' for every non-seam face."""
    ...
    for b, blk in enumerate(d.blocks):
        for fid, kind in enumerate(blk.faces):
            ...
            if np.all(np.abs(r - R_CYL) < 1e-9):
                ...
                out[(b, fid)] = roles[b]                 # 'inflow' or 'outflow'
            else:
                raise ValueError(
