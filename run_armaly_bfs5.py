...
from src.convergence import SteadyState
...
def setup(d, Re, dt, rhie_chow=False, dong=True):
    nu = U_BULK * D_H / Re
    m = MultiBlockPISO(d, nu, dt, 2, 1e-4, time_scheme="be", scheme="rotational",
...
def run(Re=100.0, dt=0.02, nsteps=3000, rhie_chow=False, dong=True, grid=None,
        report=True, save=True, restart_from=None, backend='scipy'):
    ...
    t0 = time.time()
    # Threshold follows the solver tolerance: a step-to-step change smaller than `tol` is
    # indistinguishable from solver noise. See src/convergence.py -- the old fixed 1e-9 test
    # sat four orders BELOW the actual residual floor and never fired once.
    steady = SteadyState(m.tol)
    converged = False
    for it in range(nsteps):
        div = m.step()
        if not all(np.isfinite(m.u[b]).all() for b in range(len(d.blocks))):
            return None, f"diverged at step {it + 1}", m, d
        if steady.update((m.u, m.v, m.w)):
            converged = True
            break
    ...
    if save:
        ...
        tag = (f"bfs5_Re{int(Re)}_{'dong' if dong else 'conv'}"
               f"{'_rc' if rhie_chow else ''}"
               f"{'_' + backend if backend != 'scipy' else ''}_n{d.n_cells}")
    ...
    if report:
        ...
        print(f"      {steady.report(it + 1, converged)}", flush=True)
    return xr, None, m, d
