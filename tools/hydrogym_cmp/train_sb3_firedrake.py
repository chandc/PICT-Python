...
(PICT copy of HydroGym examples/firedrake/getting_started/train_sb3_firedrake.py at 4ab9854; only change: --max-steps.)
...
def train_single_agent(args):
    ...
    from hg_znmf import CylinderZNMF   # PICT: opposed zero-net-mass-flux jets
    env_map = {
        "cylinder": firedrake.Cylinder,
        "cylinder_znmf": CylinderZNMF,
    ...
    defaults = {
        ...
        "cylinder_znmf": {"Re": 100, "dt": 1e-2, "mesh": "medium"},
    ...
    def make_env():
        ...
        env_config = {
            ...
            "max_steps": args.max_steps,   # PICT: finite episodes so Monitor/TensorBoard see episode returns
        }
...
Note: Unlike MAIA/Nek, Firedrake runs directly in Python (no MPMD required).
    ...
    parser.add_argument(
        "--env",
        required=True,
        choices=["cylinder", "cylinder_znmf", "rotary_cylinder", "pinball", "cavity", "step"],
    ...
    # Logging
    parser.add_argument("--max-steps", type=int, default=int(1e6), help="env steps per episode (HydroGym default 1e6 = one endless episode)")
