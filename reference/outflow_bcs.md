...
## Measured under vortex shedding

Everything above was assessed on steady or near-steady flows. The hard test for an outflow
condition is vortices crossing it. Square cylinder, Re = 100, 28 shedding periods after
saturation (`square_cylinder_vortex_street.md`):
...
| rms(v) one cell upstream of the outlet | 0.16 — vortices leave at full strength |

Reflections would show as sidebands or an incommensurate peak; a leaking outlet as slow envelope
modulation. Neither appears, and the harmonic ladder is clean and monotonic
(1.000 / 0.0643 / 0.0264 / 0.0042 / 0.0012).

Wake attenuation beyond x ~ 15 D is the MESH, not the boundary -- that grid deliberately resolves
