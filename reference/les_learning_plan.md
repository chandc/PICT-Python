...
### 2.2 A statistics loss — small
    ...
    L = L_stats + λ_S (1/N) Σ_n ‖S_θⁿ‖²₂          plus a clamp on S
...
Term-by-term reasoning in `les_learning_explained.md` — in particular, `λ_S ‖S‖²` is a
regulariser resolving NON-UNIQUENESS, not a second objective, and there is no term comparing S
to a "true" S because none exists.
...
*Second test, and NOT optional:* **sweep λ_S over a decade and require the recovered statistics
to be insensitive to it**, plus check whether the converged `‖S‖` is pressed against the clamp.
The penalty competes with the objective, so an optimiser can lower `L` by giving up statistical
accuracy to shrink `‖S‖` — the same trade that made `nn_fit_constants.py` BEAT THE ORACLE by
68x while getting the physics 10% worse. Without this sweep, the answer may be a property of
λ_S rather than of the flow.
