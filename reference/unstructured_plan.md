...
| T4 | **temporal** order | the time scheme alone | fix the mesh, refine `dt`, slope 2 for BDF2. **Found first order (0.85–1.00)**: the deferred non-orthogonal correction was lagged one time level. **PASSES with `n_inner=3`: 2.31 / 2.33 / 2.29** (n=128, three points before the spatial floor). Steady T5 bit-identical with and without the inner loop. `n_inner=2` measured sufficient (2.70 / 2.01) and is now the default. See `skew_unstructured_literature.md` §18 **Redone 2026-09-24 (§50, `test_utemporal.py`)**: the zero initial face flux had capped every temporal test at first order; with `init_flux()` BDF2-PISO is 1.9–2.0 (advected TGV), and the new RK3 mode (`time_scheme="rk3"`, LES plan L1a) is 2.9–3.0 in convection, 2.0 in diffusion (CN) at a 1000× smaller constant |
...
| T11 | inviscid energy conservation (port of `test_energy_conservation.py`) | skew-symmetry of the convective operator; dissipation of the full step | **done (§48)** `test_uenergy.py`: central operator conserves to round-off on quads, O(h²) anti-dissipative on tris (3e-4/turnover at 64²); full inviscid step loses 2.2%/turnover at dt 0.005 on 64² quads, falling as dt² (temporal, not spatial); first-order upwind removes 44%/turnover **RK3 (L1a, §50): 0.04%/turnover at dt 0.005, 94× below BDF2-PISO; integrator ∝ dt³, Rhie–Chow damping ∝ dt** |
| T12 | 2.5D solver: mode 0 vs the 2D RK3 solver; 3D Taylor–Green Re 100 (32²×32) | the Fourier-span extension (LES plan L2) | **done (§51)** `src/upiso25.py`, `test_utgv3d.py`: mode 0 reproduces `PISO.step_rk3` to 1.8e-15; −dE/dt equals the scheme's discrete dissipation to 0.11–0.22% at every t (0.06–0.10% at dt 0.01; 0.02–0.06% at 64²); spanwise convergence spectral (nz 4/8/16 vs 32: 1.84 / 0.118 / 5.0e-4); in-plane order 2.04 / 2.32; periodic-span cylinder (coarse butterfly × 4 planes, seeded 3D) reproduces the 2D RK3 St 0.1753 / Cd 1.4704 / Cl amp 0.3236 to every digit, 3D energy decays to 3e-27 |
| T13 | eddy-viscosity operator and closures on the 2.5D fields | LES plan L3 | **done (§52)** `src/usgs.py`, `test_usgs.py`: manufactured variable-ν divergence orders 1.97–2.00 in x/y/z; solid rotation Smagorinsky 2e-18, WALE at its analytic value to 1e-15; WALE wall exponent 2.970; Δ² scaling 4.0000. TGV Re 1600 64²×64 no model: −dE/dt peak 0.01303 at t 8.4 (DNS 0.012299 at 8.93, +5.9%), resolved ε_d peak 0.01163 (−5.5%), numerical share 11% at the peak; WALE: −dE/dt peak 0.01188 (−3.4%) at t 8.5, model share 63%, ⟨ν_t⟩/ν 1.3 (max 24), active from t = 0 (0.8ν on the laminar field), shoulder before the peak; both peaks 0.4–0.5 early |
| T14 | laminar channel by constant-mass-flow forcing; statistics; restart | LES plan L5 | **done (§53)** `test_uchannel_laminar.py`: force → 3ν and profile → Poiseuille at order 1.94/1.99; one statistics sample equals the instantaneous bin mean to 0.0; 100 steps after a restart bitwise identical |
| T15 | turbulent channel Re_τ 180, 24×80 quads × 32 modes, WALE, constant pressure gradient, DNS initial field (V2) | the LES target; the pressure two-colour mode that grew in the structured channel | **done (§54), reference SEM FOSLS DNS run02 (t 5.2–30)**: Re_τ 179.1 (−0.5%), U+ within 0.15 u_τ (+1.0%) in the log region, u_rms+ peak 2.697 vs 2.707 (−0.3%), −⟨u'v'⟩+ 0.720 vs 0.729, v/w rms −6/−7%, ⟨ν_t⟩/ν 0.19, **pressure two-colour mode 0.00% of p_rms**; 99 flow-throughs, 100 min on one core |
| T16 | 3D Taylor–Green Re 800 against the in-house SEM DNS (full history), 64/96/128 planes × modes, no model and WALE (V1) | the LES integrator plus model on a transition; the reference of the plan was corrected from "Re 1600" to the Re 800 file it actually was | **done (§58)**: peak value −4.8/−5.2% (96²/128² WALE), curve rms 5.9% (128² implicit); peak time 6% early at every resolution, dt-independent, orientation-dependent (rotated IC 7.5 vs 8.4) — the in-plane second-order dispersion; value met, time not met as posed; WALE over-dissipates the laminar phase |
| T17 | turbulent channel on triangles: split quads, hybrid cores (triangles from y⁺ 20/32/90), isotropic graded gmsh triangles | whether the non-bipartite pressure mode survives in turbulence | **done (§55)**: split quads and hybrid cores from y⁺ ≤ 32 diverge; hybrid from y⁺ 90: Re_τ 170.9, 14% two-colour pressure on the triangle pairs, zigzag Reynolds stress; graded triangles stable 20 time units but bulk +14% (momentum balance open), U⁺ +1.2 u_τ, v_rms −27%. Quads are the LES mesh |
| T18 | G4 speed: block AMG-PCG per-mode solves, whole step on CuPy | pressure to 1e-8 in < 30 iterations; ≤ 100 ms per step per 10⁵ cell-modes on the GPU | **done (§57)**: 8–19 iterations 2k–1M cells; GB10 61–73 ms per 10⁵ cell-modes (64²–384² × 64), real mesh 140; answers identical to LU |
| T19 | channel Re_τ 395 (MKM 1999 reference, 96×160 × 128 modes, minimal box) | the first LES beyond the validation Re | **done (§63)**: Re_τ 393.4 (+0.3%), U⁺ log region +0.6%, u'⁺ peak −1.9%, −u'v'⁺ −0.9%, two-colour 0.00%; streak spacing ≈ 100⁺; core departure above y⁺ 150 = the minimal-box 0.3 L_z limit (Flores & Jiménez 2010); 7.8 h on the A100 across three Colab session losses with the mean shifted to MKM's; 2.1–2.8 s/step on the GB10, to be run on the A100 |
| T20 | fluidic pinball (three cylinders) on HydroGym's medium triangle mesh, Re 30 and 100, against their Firedrake P2-P1 from the same start | a genuinely multi-body unstructured case; the reference's dependence on its start | **done (§62)**: Re 100 shedding: St 0.1110 vs 0.1112, total C_D +1.0%, per-cylinder drag +0.9–1.2%, lift amplitudes within 3%; Re 30: both solvers find the same growing far-wake oscillation (f 0.081, amplitudes 0.06) from the impulsive start while the Newton steady state is the symmetric branch — lift histories coincide, drag +1.1%. Meshes `meshes/pinball_medium.msh`, `pinball_fine.msh`; drivers `run_upinball.py`, `tools/hydrogym_cmp/hg_pinball.py` |
| T21 | V3: cylinder Re 3900 on a purpose-built quad butterfly (60k cells, wall 0.003 D) × 64 planes, WALE, A100 notebook | St 3%, C_D 5%, L_r 10% of the Parnaudeau band | **prepared (§64)**: mesh, driver with statistics/restart, notebook `tools/a100/A100_cylinder_re3900.ipynb`, GB10 profile 3.1 s/step (~0.85 on the A100, 15–20 h); WALE must start at t = 2 |
