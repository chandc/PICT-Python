...
| T11 | inviscid energy conservation (port of `test_energy_conservation.py`) | skew-symmetry of the convective operator; dissipation of the full step | **done (§48)** `test_uenergy.py`: central operator conserves to round-off on quads, O(h²) anti-dissipative on tris (3e-4/turnover at 64²); full inviscid step loses 2.2%/turnover at dt 0.005 on 64² quads, falling as dt² (temporal, not spatial); first-order upwind removes 44%/turnover |
